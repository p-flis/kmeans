...
void initCentroids(float* inputX, float* inputY, float* inputZ,
    float* meansX, float* meansY, float* meansZ)
{
    int randk = rand()%K;
    meansX[0] = inputX[randk];
    meansY[0] = inputY[randk];
    meansZ[0] = inputZ[randk];
    for(int i = 1; i < K; i++)
    {
        float bestd = -1;
        int bestind = 0;
        ...
        meansX[i] = inputX[bestind];
        meansY[i] = inputY[bestind];
        meansZ[i] = inputZ[bestind];
    }
...
void runTest(int argc, char **argv)
{
    ...
    initCentroids(inputX, inputY, inputZ, meansX, meansY, meansZ);
